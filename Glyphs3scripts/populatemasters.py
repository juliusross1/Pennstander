...
## Fixme: Fail nicely here if masters are not available
	...
	def insert_mathic(populatinglayers):			
			...
			if mathic_anchor:
				print('found math.ic in Slant',glyph)
				new_anchor = GSAnchor()
				new_anchor.name = "math.ic"
				new_anchor.position = NSPoint(populatinglayers['Thin'].width, 0)
				
				#print('adding anchor to Thin',new_anchor,new_anchor.position)
				populatinglayers['Thin'].anchors.append(new_anchor)
				
				new_anchor2 = GSAnchor()
				new_anchor2.name = "math.ic"
				
				new_anchor2.position = NSPoint(populatinglayers['Black'].width, 0)
				#print('adding anchor to Black',new_anchor2,new_anchor2.position)
	...
	for glyph in glyphlist:
		...
		if ('bold-math' not in glyph.name) and ('italic-math' not in glyph.name) and ('bolditalic-math' not in glyph.name):	
			if glyph.name+'italic-math' in font.glyphs:
				...
				# If an anchor called math.ic exists in the Slant layer then create one in the Thin and Black layer
				insert_mathic(populatinglayers)

				for element in action['letter']:
					copy_master_content2(populatinglayers[element[1]],glyph,masterids[element[0]])

			else: #Else we are dealing with a math symbol so there is no Slant expected
